...
import com.workflow.iptv.completion.domain.service.IptvSiteService;
...
public class WorkflowStepRequestConsumer {
   ...
   private final IptvSiteService iptvSiteService;

    @KafkaListener(topics = "${kafka.topics.iptv.step.request}")
    public void handleIptvStepResponse(WorkflowStepEvent event) {
        log.debug("[IPTV(SITE)]Received step response: {}", event);
        try {
            iptvSiteService.doProcess(event);
