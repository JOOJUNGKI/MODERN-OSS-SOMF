...
import com.workflow.iptv.completion.domain.service.IptvCompletionService;
...
public class WorkflowStepRequestConsumer {
   ...
   private final IptvCompletionService iptvCompletionService;

    @KafkaListener(topics = "${kafka.topics.iptv.step.request}")
    public void handleIptvStepResponse(WorkflowStepEvent event) {
        log.debug("[IPTV(COMPLETION)]Received step response: {}", event);
        try {
            iptvCompletionService.doProcess(event);
