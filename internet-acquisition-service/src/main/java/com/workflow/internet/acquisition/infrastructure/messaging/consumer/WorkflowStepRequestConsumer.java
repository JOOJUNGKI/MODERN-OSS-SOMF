...
package com.workflow.internet.acquisition.infrastructure.messaging.consumer;

import com.workflow.common.event.WorkflowStepEvent;
import com.workflow.internet.acquisition.domain.service.InternetAcquisitionService;
...
public class WorkflowStepRequestConsumer {
   ...
   private final InternetAcquisitionService internetAcquisitionService;

    @KafkaListener(topics = "${kafka.topics.internet.step.request}")
    public void handleInternetStepResponse(WorkflowStepEvent event) {
        log.debug("[INTERNET(ACQUISITION)]Received step response: {}", event);
        try {
            internetAcquisitionService.doProcess(event);
